/**
 * Truck Loader — used as Suspense fallback + route-level loading state.
 * Pure SVG + globals.css keyframes (no styled-components dep).
 */
  ...
  return (
    ...
    >
      <div className="relative w-[200px] h-[100px] flex flex-col items-center justify-end overflow-hidden">
        ...
        {/* Tires */}
        <div className="absolute bottom-0 w-[130px] flex items-center justify-between pl-[15px] pr-[10px]">
          {[0, 1].map((i) => (
            <svg
              key={i}
              xmlns="http://www.w3.org/2000/svg"
        ...
        {/* Road line — base */}
        <div className="w-full h-[1.5px] bg-[#282828] relative self-end rounded-[3px]">
          {/* Two moving road segments */}
          <span
            aria-hidden
            className="absolute h-full bg-[#282828] rounded-[3px] -right-1/2 w-5 border-l-[10px] border-l-white animate-truck-road"
          />
          <span
            aria-hidden
            className="absolute h-full bg-[#282828] rounded-[3px] w-[10px] border-l-4 border-l-white animate-truck-road"
            style={{ right: "-65%" }}
          />
        </div>

        {/* Lamp post sliding past */}
        <svg
          ...
          fill="#000000"
          xmlSpace="preserve"
          className="absolute bottom-0 -right-[90%] h-[90px] animate-truck-road"
          aria-hidden="true"
        >
      ...
      {label && (
        <p className="font-mono text-xs uppercase tracking-widest text-brand-ink-soft">
          {label}
        </p>
      )}
    </div>
